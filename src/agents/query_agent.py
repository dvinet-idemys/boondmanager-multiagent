...
def create_query_agent(
    ...
    """Create a standalone query agent with all data-fetching subagents.

    Args:
        model: LLM model to use. If None, uses default from llm_config.

    Returns:
        Configured LangGraph agent ready to handle data queries.
    """
    if model is None:
        model = get_llm()
...
async def query_agent_tool(prompt: str):
    ...
    Args:
        prompt (str): Open-ended question requesting worker/project data with worker name and time period.
    """

    async for message in create_query_agent().astream({"messages": [("user", prompt)]}):
    ...
    ret = await create_query_agent().ainvoke({"messages": prompt})

    return ret.get("messages", [""])[-1].content


async def demo_query_agent():
    """Demo function to test the query agent with example queries."""
    print("=== BoondManager Query Agent Demo ===\n")

    agent = create_query_agent()

    # Example queries
    queries = [
        "How many days did worker Elodie LEGUAY work on project modernisation in September 2025?",
        "How many days did worker Didier Geig work on project modernisation in September 2025?",
        "What was the total cost for worker Jon LEVIN on the project where he worked the least in September 2025?",
        "What was the total cost for worker Jon LEVIN on the project where he worked the most in September 2025?",
        "What was LEVIN Jon's average daily rate for project Tps partiel?",
    ]
