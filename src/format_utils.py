...
def format_messages(messages, pad_left: int = 0):
    ...
    for m in messages:
        ...
        if msg_type == "Human":
            console.print(
                Padding(
                    Panel(content, title="🧑 Human", border_style="blue"),
                    pad=(0, 0, 0, pad_left),
                )
            )
        elif msg_type == "Ai":
...
def format_message(message, pad_left: int = 0):
    return format_messages([message], pad_left)
